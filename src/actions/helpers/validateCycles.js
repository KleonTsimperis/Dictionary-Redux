...
export const validateCycles = (array, name) => {
  ...
  for (let i = 0; i < array.length; i++){
    for (let j = 0; j < array.length; j++){
      if ((array[i].domainTerm === array[j].rangeTerm && array[i].rangeTerm !== array[j].domainTerm && i !== j) ||
          (array[i].rangeTerm === array[j].domainTerm && array[i].domainTerm !== array[j].rangeTerm && i !== j) ||
          (array[i].rangeTerm !== array[j].domainTerm && array[i].domainTerm === array[j].rangeTerm && i !== j) ||
          (array[i].rangeTerm !== array[j].domainTerm && array[i].domainTerm === array[j].rangeTerm && i !== j) ||
          (array[i].rangeTerm === array[j].domainTerm) || (array[i].rangeTerm !== array[j].domainTerm) || array.length <= 1){
